...
let downloadAllStopFlag = false;
let _downloadAllFetchController = new AbortController();
// Resolvers woken when a graceful stop or fatal abort is requested, so workers,
// extraction tabs, and CAPTCHA waits do not remain blocked.
let _downloadAllAbortResolvers = [];
function _wakeDownloadAllWaiters() {
...
function _signalDownloadAllStop() {
  ...
  _cancelDownloadAllNetwork();
  _wakeDownloadAllWaiters();
}
...
function _resetDownloadAllAbort() {
  ...
  _downloadAllAbortResolvers = [];
  _downloadAllFetchController = new AbortController();
}
function downloadAllShouldStop() {
  return downloadAllAbortFlag || downloadAllStopFlag;
}
function downloadAllNetworkSignal() {
  return _downloadAllFetchController.signal;
}
function makeDownloadAllStoppedError() {
  const error = new Error('Download All was cancelled.');
  error.name = 'DownloadAllStoppedError';
  error.code = 'DOWNLOAD_ALL_STOPPED';
  return error;
}
function isDownloadAllStoppedError(error) {
  return !!error && (error.code === 'DOWNLOAD_ALL_STOPPED' || error.name === 'DownloadAllStoppedError');
}
...
function _downloadAllAbortPromise() {
  if (downloadAllShouldStop()) return Promise.resolve();
  return new Promise((res) => _downloadAllAbortResolvers.push(res));
}

/** Prepared archive retained only while a cancelled Save dialog awaits Save again / Discard. */
...
function withDownloadAllSessionLock(task) {
  const result = _downloadAllSessionLock.then(task);
  _downloadAllSessionLock = result.catch(() => {});
  return result;
}

function downloadAllSessionOwnedBy(session, tabId, tabUrl = '', sessionId = '') {
  return !!session && (session.originTabId == null || session.originTabId === tabId) &&
    (!sessionId || session.sessionId === sessionId) &&
    (!downloadAllSessionSlug(session) || downloadAllSessionMatchesUrl(session, tabUrl));
}

function runDownloadAllRequest(...args) {
  if (_downloadAllRun) throw new Error('Another Download All operation is still stopping.');
  const run = {};
  _downloadAllRun = run;
  _resetDownloadAllAbort();
  // Reserve the runner and create its session before yielding to another command.
  return handleDownloadAllRequest(...args).catch((error) => {
    _signalDownloadAllAbort();
    if (downloadAllSession && downloadAllSession.active) {
      notifyDownloadAllError(downloadAllSession.originTabId, error, {
        errorTitle: 'Download stopped unexpectedly.', errorKind: 'pipeline', cause: error,
      });
    }
    throw error;
  }).finally(() => {
...
function downloadActivitySnapshot() {
  return {
    downloadAllActive: !!(_downloadAllRun || (downloadAllSession && downloadAllSession.active)),
...
function startDownloadAllSession({
  ...
  sessionId = crypto.randomUUID(), revision = 0,
}) {
  const now = Date.now();
...
function updateDownloadAllSessionLog(progress) {
  ...
  if (phase === 'extracting') {
    ...
    const retrying = progress.challengeState === 'retrying';
    text = retrying
...
function recordDownloadAllCancelling() {
  if (!downloadAllSession || !downloadAllSession.active || downloadAllSession.status === 'cancelling') return;
  const progress = {
    action: 'downloadAllProgress',
    phase: 'cancelling',
...
function recordDownloadAllTerminal(status, patch = {}) {
  ...
  Object.assign(downloadAllSession, {
    ...
    lastProgress: null,
    ...patch,
    originTabId: downloadAllSession.originTabId ?? patch.originTabId ?? null,
    updatedAt: Date.now(),
  });
  persistDownloadAllSession(true);
}
...
function queueDownloadAllStorage(task) {
  _dlStorageQueue = _dlStorageQueue.then(task).catch(() => {});
  return _dlStorageQueue;
}
...
function normalizeDownloadAllResumeChapters(chapters) {
  ...
  return chapters.map((chapter) => {
    const scanlator = String(chapter && (chapter.scanlator || chapter.group) || '').trim();
    return {
      chapterUrl: String(chapter && chapter.chapterUrl || ''),
      chapterLabel: String(chapter && chapter.chapterLabel || ''),
      scanlator,
      group: scanlator,
      groupId: String(chapter && chapter.groupId || '').trim(),
      chapterTitle: String(chapter && chapter.chapterTitle || '').trim(),
    };
...
function _serializeSession(s) {
  ...
  const keys = ['active', 'status', 'originTabId', 'mangaName', 'zipName', 'totalChapters',
    ...
    'seriesSlug', 'sessionId', 'revision', 'cancelRequested',
...
function persistDownloadAllSession(force = false, includeResumeData = false) {
  const session = downloadAllSession;
  if (!session) return;
  ...
  const write = () => {
    ...
    try {
      ...
      const values = { [keys.session]: _serializeSession(session) };
      if (includeResumeData || !session.resumeData) {
        values[keys.resume] = session.resumeData || null;
      }
      const snapshot = JSON.parse(JSON.stringify(values));
      queueDownloadAllStorage(() => chrome.storage.local.set(snapshot));
    } catch (_) {}
  };
...
function clearPersistedSession(session = downloadAllSession) {
  ...
  const keys = downloadAllStorageKeys(session);
  return queueDownloadAllStorage(() => chrome.storage.local.remove([keys.session, keys.resume]));
}
function clearPersistedDownloadAllResume(session = downloadAllSession) {
  const keys = downloadAllStorageKeys(session);
  if (session) session.resumeData = null;
  return queueDownloadAllStorage(() => chrome.storage.local.remove(keys.resume));
}
function clearLegacyPersistedDownloadAllSession() {
  return queueDownloadAllStorage(() => chrome.storage.local.remove([DL_SESSION_KEY, DL_RESUME_KEY]));
}
async function loadPersistedSession(tabUrl = '') {
  try {
    await _dlStorageQueue;
    ...
    throw new Error('The saved download checkpoint could not be read. Please try again.');
  }
}
...
function prepareInterruptedDownloadAllSession(stored, tabId) {
  ...
  if (stored && (stored.cancelRequested || previousStatus === 'cancelling' || previousStatus === 'cancelled')) {
    Object.assign(stored, {
      active: false, status: 'cancelled', phase: 'cancelled', resumeData: null,
      canRetryZip: false, canRetrySave: false, canResumeDownload: false,
      lastProgress: null, updatedAt: Date.now(),
    });
    return stored;
  }
...
async function getDownloadAllSessionForTabAsync(tabId, tabUrl = '') {
  return withDownloadAllSessionLock(() => loadDownloadAllSessionForTab(tabId, tabUrl));
}

async function loadDownloadAllSessionForTab(tabId, tabUrl = '') {
  const requestedSlug = downloadAllTabSlug(tabUrl);
  ...
  if (!_downloadAllRun && (!downloadAllSession || (!downloadAllSession.active && !currentMatchesTitle))) {
    ...
    if (stored) {
      ...
      if (lostRuntimeState) {
        prepareInterruptedDownloadAllSession(stored, stored.originTabId ?? (sameTitle ? tabId : null));
      }
  ...
  // Finished runs are delivered to the current UI once, not reopened on every visit.
  if (downloadAllSession && ['done', 'cancelled'].includes(downloadAllSession.status)) {
    if (!_downloadAllRun) {
      const finished = downloadAllSession;
      downloadAllSession = null;
      await clearPersistedSession(finished);
    }
    return null;
  }

  if (downloadAllSession && !downloadAllSession.active && !downloadAllSession.canResumeDownload &&
      Date.now() - (downloadAllSession.updatedAt || 0) > DOWNLOAD_ALL_TERMINAL_SESSION_TTL_MS) {
    const expired = downloadAllSession;
    downloadAllSession = null;
    await clearPersistedSession(expired);
    return null;
  }
  ...
  if (
    ...
    if (downloadAllSession.originTabId != null) {
      try {
        const owner = await chrome.tabs.get(downloadAllSession.originTabId);
        if (downloadAllSessionMatchesUrl(downloadAllSession, owner && owner.url)) return null;
      } catch (_) {}
    }
    downloadAllSession.originTabId = tabId;
...
function dismissDownloadAllSessionForTab(tabId, tabUrl = '', sessionId = '') {
  if (!downloadAllSession) return;
  if (sessionId && downloadAllSession.sessionId !== sessionId) return;
...
function notifyDownloadAllProgress(originTabId, progress) {
  if (!recordDownloadAllProgress(progress)) return;
  if (typeof progress.completed === 'number' && progress.totalChapters) {
    setProgressBadge(progress.completed, progress.totalChapters);
  }
  notifyDownloadAllMessage(originTabId, { action: 'downloadAllProgress', ...progress });
}

function notifyDownloadAllMessage(originTabId, message) {
  const session = downloadAllSession;
  notifyTab(session ? session.originTabId : originTabId, {
    ...message, sessionId: session && session.sessionId,
    revision: session && session.revision, seriesSlug: downloadAllSessionSlug(session),
  });
}
...
function notifyDownloadAllDone(originTabId, zipName, warning = '') {
  ...
  clearPersistedDownloadAllResume();
  restoreIdleBadge();
  notifyDownloadAllMessage(originTabId, message);
}
...
function notifyDownloadAllSaveCancelled(originTabId, filename, zipPart = 1, finalPart = true) {
  ...
  if (downloadAllSession) {
    Object.assign(downloadAllSession, {
      ...
      phase: 'saving',
      originTabId: downloadAllSession.originTabId ?? originTabId,
...
function notifyDownloadAllCancelled(originTabId, details = {}) {
  const message = {
    action: 'downloadAllCancelled',
    zipName: String(details.zipName || ''),
    savedChapters: Math.max(0, Math.floor(Number(details.savedChapters) || 0)),
    warning: String(details.warning || ''),
  };
  recordDownloadAllTerminal('cancelled', {
    resumeData: null,
    originTabId,
    doneZipName: message.zipName,
    savedChapters: message.savedChapters,
...
async function resumeDownloadAllFromCheckpoint(originTabId, tabUrl = '', sessionId = '') {
...
async function cancelDownloadAllForTab(tabId, tabUrl = '', sessionId = '') {
  const session = await loadDownloadAllSessionForTab(tabId, tabUrl);
  if (!session) return { ok: true, session: null };
  if (!downloadAllSessionOwnedBy(session, tabId, tabUrl, sessionId)) {
    return { ok: false, error: 'This download belongs to another tab or session.' };
  }
  if (!_downloadAllRun || !session.active) {
    dismissDownloadAllSessionForTab(tabId, tabUrl, sessionId);
    return { ok: true, session: null };
  }
  _signalDownloadAllStop();
  recordDownloadAllCancelling();
  resolvePendingArchiveSaveDecision(false);
  notifyDownloadAllMessage(tabId, session.lastProgress);
  return { ok: true, session: _serializeSession(session) };
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === 'downloadAllChapters') {
    const originTabId = sender.tab?.id ?? null;
    withDownloadAllSessionLock(async () => {
        const existing = await loadDownloadAllSessionForTab(originTabId, sender.tab?.url || '');
        if (existing && (
        ...
        if (_downloadAllRun || (downloadAllSession && downloadAllSession.active)) {
          sendResponse({
            ok: false,
        ...
        if (!Array.isArray(message.chapters) || !message.chapters.length ||
            message.chapters.some((chapter) => !chapter || !chapter.chapterUrl || !chapter.chapterLabel)) {
        ...
        sendResponse({ ok: true, sessionId: downloadAllSession.sessionId });
      })
      .catch((error) => {
  ...
  if (message.action === 'cancelDownloadAll') {
    withDownloadAllSessionLock(() => cancelDownloadAllForTab(
      sender.tab?.id ?? null, sender.tab?.url || '', message.sessionId || ''
  ...
  if (message.action === 'resumeDownloadAll') {
    const originTabId = sender.tab?.id ?? null;
    withDownloadAllSessionLock(() => resumeDownloadAllFromCheckpoint(
      originTabId, sender.tab?.url || '', message.sessionId || ''
    ))
  ...
  if (message.action === 'retryArchiveSave') {
    ...
    const resumed = belongsToSender &&
      downloadAllSessionOwnedBy(downloadAllSession, originTabId, sender.tab?.url || '', message.sessionId || '') &&
      resolvePendingArchiveSaveDecision(true);
  ...
  if (message.action === 'abandonArchiveSave') {
    ...
    const ownsSession = belongsToSender &&
      downloadAllSessionOwnedBy(downloadAllSession, originTabId, sender.tab?.url || '', message.sessionId || '');
    if (ownsSession) _pendingZip.dismissAfterAbandon = true;
    const abandoned = ownsSession && resolvePendingArchiveSaveDecision(false);
  ...
  if (message.action === 'dismissDownloadAllSession') {
    const originTabId = sender.tab?.id ?? null;
    withDownloadAllSessionLock(async () => {
      await loadDownloadAllSessionForTab(originTabId, sender.tab?.url || '');
      dismissDownloadAllSessionForTab(originTabId, sender.tab?.url || '', message.sessionId || '');
      await _dlStorageQueue;
      return { ok: true };
    }).then(sendResponse).catch((error) => sendResponse({ ok: false, error: error.message }));
    return true;
...
async function handlePendingDownloadTabUpdated(tabId, changeInfo) {
  ...
  try {
    ...
    if (challenge.challenged) pending.challengeEncountered = true;
    ...
    if (!Array.isArray(images) || images.length === 0) {
      if (pending.challengeEncountered && !pending.challengeExtractionRetried) {
        pending.challengeExtractionRetried = true;
        pending.handling = false;
        notifyTab(originTabId, { action: 'downloadChallenge', chapterUrl, state: 'retrying' });
...
async function downloadImagesAsZip({ images, chapterUrl, zipName, originTabId, cfg, options }) {
  ...
  const mangaName = (options && options.mangaName) || (opts.seriesMeta && opts.seriesMeta.title) || '';
  const requestedEntry = String(zipName || '').replace(/\.(zip|cbz|pdf)$/i, '');
  const outputBase = isCbz
    ? buildCbzFileBase(opts, chapterLabel, mangaName, options, requestedEntry)
    : zipName;
  const outName = sanitizeFilename(outputBase, ext);
  ...
  const destinationWarning = delivery.pathFallback
    ? ' The browser saved it without the configured subfolder.'
    : '';

  cdlLog(accepted ? 'ok' : 'warn',
    `${ext.toUpperCase()} ${delivery.confirmed ? 'saved' : accepted ? 'sent to mobile downloads' : 'handed to browser'}: ${savedName} (${images.length} images).${destinationWarning}`);
  if (delivery.confirmed) await recordSuccessfulDownloadForReview();
  notifyTab(originTabId, {
    action: 'downloadDone',
    chapterUrl,
    warning: destinationWarning.trim(),
  });
...
function waitForAbortableDelay(delayMs, signal) {
  ...
  if (signal.aborted) return Promise.reject(makeDownloadAllStoppedError());
  return new Promise((resolve, reject) => {
    ...
    const onAbort = () => {
      clearTimeout(timer);
      reject(makeDownloadAllStoppedError());
    };
    signal.addEventListener('abort', onAbort, { once: true });
  });
}

async function waitForImageHostCooldown(src, signal) {
  const host = imageHostKey(src);
  if (!host) return;
...
async function saveGeneratedArchive({
  ...
  let mobile = false;
  try { mobile = await isMobileBrowserPlatform(); } catch (_) {}
  const basename = String(filename || '').split(/[\\/]/).pop() || 'download.zip';
  const canRetryWithoutFolder = mobile && basename !== filename &&
    startOrTransferError && startOrTransferError.downloadPhase === 'start';
  if (canRetryWithoutFolder) {
    try {
      ...
      const item = await startAndConfirmBrowserDownload(
        { url, filename: basename, saveAs: false },
        reportSave ? { onProgress: reportSave } : undefined
      );
      revoke();
      return {
  ...
  if (!payload) {
    ...
    payload = await zip.generateAsync({ type: 'base64', compression: 'STORE' });
  }
  revoke();
  const fallbackFilename = mobile ? basename : filename;
  const response = await chrome.tabs.sendMessage(originTabId, {
    action: 'triggerDownload', base64: payload, filename: fallbackFilename,
  });
  ...
  return {
    ...
    pathFallback: fallbackFilename !== filename,
  };
}
...
function downloadTargetFilename(filename, subfolder) {
  if (typeof CDLSettings !== 'undefined' && CDLSettings.withDownloadSubfolder) {
    return CDLSettings.withDownloadSubfolder(filename, subfolder);
  }
  return String(filename || '').split(/[\\/]/).pop() || 'download.zip';
}
...
function chapterNamingParts(opts, chapterLabel, mangaName, source, entry) {
  source = source || {};
  const match = String(chapterLabel || '').match(/^Ch([\d.]+)(.*)/i);
  const number = match ? match[1] : String(chapterLabel || '').replace(/^Ch/i, '');
  const scanlator = String(source.scanlator || source.group || '').trim();
  const meta = opts && opts.seriesMeta || {};
  return {
    entry: entry || '',
    manga: mangaName || meta.title || '',
    chapter: number,
    label: chapterLabel || '',
    ...
    slug: opts && opts.slug || meta.slug || '',
  };
}

function buildCbzFileBase(opts, chapterLabel, mangaName, source, entry) {
  const fallback = String(entry || chapterLabel || 'Chapter').split('/').pop() || 'Chapter';
  if (typeof CDLSettings === 'undefined') return fallback;
  return CDLSettings.renderName(
    opts && opts.cbzFileTpl || '{entry}',
    chapterNamingParts(opts, chapterLabel, mangaName, source, fallback),
    160
  );
}

function buildCbzEntryName(opts, chapter, mangaName) {
  const legacyEntry = buildChapterEntryName(opts, chapter.chapterLabel, mangaName);
  const splitAt = legacyEntry.lastIndexOf('/');
  const parent = splitAt >= 0 ? legacyEntry.slice(0, splitAt + 1) : '';
  const entry = splitAt >= 0 ? legacyEntry.slice(splitAt + 1) : legacyEntry;
  return parent + buildCbzFileBase(opts, chapter.chapterLabel, mangaName, chapter, entry);
}

function uniqueChapterEntryName(zip, entryBase, extension, chapterLabel) {
  let candidate = entryBase;
  let suffix = 2;
  ...
  while (zip.file(`${candidate}.${extension}`)) {
    candidate = `${entryBase}-${chapterSuffix}${suffix > 2 ? `-${suffix}` : ''}`;
    suffix++;
  }
  return candidate;
}
...
async function addChapterToOuter(zip, r, opts, mangaName) {
  ...
  if (opts.format === 'cbz') {
    ...
    if (opts.pushLib) {
      const fileBase = cbzEntry.split('/').pop();
...
function cloudflareChapterLocationMatches(actualUrl, expectedUrl) {
  ...
  try {
    ...
    return String(actualUrl || '').split('#')[0] === String(expectedUrl || '').split('#')[0];
  }
}

function raceCloudflareCancellation(task, navigation) {
  const cancelPromise = navigation && navigation.cancelPromise;
  if (!cancelPromise) return task;
  return Promise.race([
    task,
    Promise.resolve(cancelPromise).then(() => { throw makeDownloadAllStoppedError(); }),
  ]);
}

async function reloadCloudflareChapterTab(tabId, navigation) {
  const expectedUrl = navigation && navigation.expectedChapterUrl;
  if (expectedUrl) {
    await chrome.tabs.update(tabId, { url: withExtractMarker(expectedUrl) });
  } else {
    await chrome.tabs.reload(tabId);
  }
}

async function waitForCloudflareChallengeClear(tabId, deadline, navigation = {}) {
  let restoredExpectedChapter = false;
  while (Date.now() < deadline) {
    if (navigation.cancelled && navigation.cancelled()) throw makeDownloadAllStoppedError();
    let tab;
    try { tab = await chrome.tabs.get(tabId); }
    catch (_) { throw new Error('The Cloudflare verification tab was closed.'); }
    if (tab && tab.status !== 'complete') {
      await raceCloudflareCancellation(
        new Promise((resolve) => setTimeout(resolve, CLOUDFLARE_CHALLENGE_POLL_MS)),
        navigation
      );
      continue;
    }
...
async function coordinateCloudflareChallenge(tabId, navigation = {}) {
  ...
  if (_cloudflareChallengeGate && _cloudflareChallengeGate.tabId !== tabId) {
    ...
    await raceCloudflareCancellation(_cloudflareChallengeGate.promise, navigation);
    ...
    await raceCloudflareCancellation(turn, navigation);
    try {
      reportCloudflareChallenge(onState, 'retrying');
      await reloadCloudflareChapterTab(tabId, navigation);
  ...
  if (!_cloudflareChallengeGate) {
    const promise = (async () => {
      ...
      try {
        await waitForCloudflareChallengeClear(tabId, automaticDeadline, navigation);
      ...
      await waitForCloudflareChallengeClear(
        tabId,
        Date.now() + CLOUDFLARE_CHALLENGE_TIMEOUT_MS,
        navigation
      );
  ...
  try {
    await raceCloudflareCancellation(gate.promise, navigation);
  } finally {
  ...
  try {
    ...
    throw new Error('The chapter tab closed after Cloudflare verification.');
  }
}
...
async function extractFromTab(url, cfg, navigation = {}) {
  ...
  opened.navigation.cancelPromise = navigation.cancelPromise;
  opened.navigation.cancelled = navigation.cancelled;
  ...
  return new Promise((resolve, reject) => {
    ...
    if (opened.navigation.cancelPromise) {
      Promise.resolve(opened.navigation.cancelPromise).then(() => {
        settle(reject, makeDownloadAllStoppedError());
      }).catch(() => {});
    }
    ...
    const onUpdated = async (updatedId, changeInfo, updatedTab) => {
      ...
      try {
        ...
        if ((!Array.isArray(images) || images.length === 0) &&
          ...
          if (!opened.navigation.challengeExtractionRetried) {
            opened.navigation.challengeExtractionRetried = true;
            reportCloudflareChallenge(opened.navigation.onChallenge, 'retrying');
            handling = false;
            armTimer();
            await reloadCloudflareChapterTab(tabId, opened.navigation);
            return;
          }
          const error = new Error('Cloudflare verification cleared, but the chapter reader did not load.');
          error.code = 'CHALLENGE_RECOVERY_EMPTY';
          throw error;
        }
        settle(resolve, images);
      } catch (err) {
        settle(reject, err);
      }
    };
...
async function handleDownloadAllRequest(
  chapters, mangaName, zipName, originTabId, options, resumeState = null, sessionId = crypto.randomUUID()
) {
  ...
  startDownloadAllSession({
    sessionId: resumeState && resumeState.sessionId || sessionId,
    revision: resumeState && resumeState.revision || 0,
    originTabId,
  ...
  let destinationFallbacks = 0;
  let acceptedChapterCount = Math.max(0, Number(resumeData.terminalCounts.done) || 0);
  ...
  let archiveAdmissionPaused = false;
  let archiveAdmissionWaiters = [];
  const resumeArchiveAdmission = () => {
    archiveAdmissionPaused = false;
    const waiters = archiveAdmissionWaiters;
    archiveAdmissionWaiters = [];
    for (const resolve of waiters) resolve();
  };

  const notify = (extra) => {
    if (downloadAllSession !== session || !session.active) return;
    const phase = extra && extra.phase;
    if (downloadAllStopFlag && chapterProgressPhases.has(phase)) return;
  ...
  const flushDeferredChapterProgress = () => {
    if (archivePresentationActive || pdfPresentationActive || downloadAllShouldStop() || !session.active) return 0;
  ...
  const saveCurrentZipPart = async (
    ...
    const pending = {
      ...
      downloadSubfolder: cfg['output.downloadSubfolder'] || '',
      ...
      notifyDone: false,
      cancelDetails: {
        zipName: savedZipNames.length === 1
          ? savedZipNames[0]
    ...
    if (accepted) acceptedChapterCount += partChapters;
    cdlLog(accepted ? 'ok' : 'warn',
  ...
  const downloadChapter = async (i) => {
    ...
    const { chapterUrl, chapterLabel } = chapter;
    const scanlator = String(chapter.scanlator || chapter.group || '').trim();
    const sourceMetadata = {
      scanlator,
      group: scanlator,
      groupId: String(chapter.groupId || '').trim(),
    ...
    for (let attempt = 0; attempt <= chapterRetries; attempt++) {
      ...
      try {
        ...
        if (downloadAllShouldStop() || isDownloadAllStoppedError(e)) return cancelledResult();
        extractErr = e;
        images = null;
    ...
    for (let retryAttempt = 1;
      retryAttempt <= chapterRetries && failedPages.size && !downloadAllShouldStop();
      retryAttempt++) {
    ...
    if (opts.format === 'pdf') {
      try {
        ...
        if (downloadAllShouldStop() || !pdfBytes) return cancelledResult();
        return {
          index: i, chapterUrl, chapterLabel, ...sourceMetadata, files: [], pdfBytes,
    ...
    return {
      index: i, chapterUrl, chapterLabel, ...sourceMetadata, files, bytes,
      imagesTotal: ordered.length, imagesSaved: files.length, status: 'done',
      hadRetries: encounteredRetries,
    };
  };
  ...
  const waitForWindow = async () => {
    while (!downloadAllShouldStop() && (cursor - packedCount) >= concurrency) {
      await Promise.race([new Promise((res) => windowWaiters.push(res)), abortPromise]);
    }
  };
  const waitForArchiveAdmission = async () => {
    while (archiveAdmissionPaused && !downloadAllShouldStop()) {
      await Promise.race([
        new Promise((resolve) => archiveAdmissionWaiters.push(resolve)),
        abortPromise,
      ]);
    }
  };
  ...
  const worker = async () => {
    while (true) {
      ...
      if (rateMode !== 'off' && !downloadAllShouldStop() && cursor < chapters.length) {
        await Promise.race([sleep(rateDelay), abortPromise]);
      }
    }
  };
  ...
  const settle = (task) => task.catch((error) => {
    _signalDownloadAllAbort();
    wakeWindow();
    resumeArchiveAdmission();
    throw error;
  });
  const settled = await Promise.allSettled([packer(), ...workers].map(settle));
  ...
  if (downloadAllStopFlag) {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      const estimatedBytes = result.pdfBytes && result.pdfBytes.byteLength
        ? result.pdfBytes.byteLength
        : Math.max(0, Number(result.bytes) || 0);
      const projectedReason = downloadAllProjectedPartSplitReason(
        zipPartChapters,
        zipPartBytes,
        estimatedBytes,
      ...
      try {
        const added = await withExtensionKeepAlive(
          () => addChapterToOuter(zip, result, opts, mangaName)
        );
        zipPartChapters++;
        zipPartBytes += added;
...
async function _doZipAndSave(pending) {
  ...
  try {
    ...
    while (!delivery) {
      try {
        ...
        if (!shouldRetry) {
          ...
          if (dismissAfterAbandon) dismissDownloadAllSessionForTab(pending.originTabId);
          return null;
        }
    ...
    const accepted = isArchiveDeliveryAccepted(delivery);
    const deliveredFilename = delivery.filename || filename;
    if (accepted) {
    ...
    return {
      ...
      pathFallback: delivery.pathFallback === true,
    };
  } catch (err) {
    releasePendingGeneratedArchive(pending);
    if (_pendingZip === pending) _pendingZip = null;
...
async function postponeAutoDownload(slug, mangaName, newOnes) {
  // Drop these keys from the baseline so the next subscription check retries.
...
async function autoDownloadNew(slug, mangaName, newOnes, cfg, seriesTotal) {
  if (_downloadAllRun || (downloadAllSession && downloadAllSession.active)) {
    await postponeAutoDownload(slug, mangaName, newOnes);
    return;
  }
  ...
  const task = await withDownloadAllSessionLock(() => {
    if (_downloadAllRun || (downloadAllSession && downloadAllSession.active)) return null;
    const running = runDownloadAllRequest(chapters, mangaName, zipName, null, options);
    return { running, session: downloadAllSession };
  });
  if (!task) {
    await postponeAutoDownload(slug, mangaName, newOnes);
    return;
  ...
  if (cfg['subscribe.notify'] && chrome.notifications) {
    try {
      chrome.notifications.create(`cdlsub:${slug}`, {
        type: 'basic',
        iconUrl: chrome.runtime.getURL('icons/icon128.png'),
        title: `Auto-download ${task.session.status === 'done' ? 'finished' : 'stopped'} — ${mangaName}`,
        message: task.session.status === 'done'
          ? task.session.warning || `${chapters.length} new chapter(s) saved to your downloads folder.`
          : task.session.error || 'The download was cancelled. Previously saved chapters are kept.',
        priority: 0,
      });
    } catch (_) {}
  }
